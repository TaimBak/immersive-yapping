...
TimeDomainConvolver::TimeDomainConvolver(const std::vector<float> &inputIR)
    : ir(inputIR), irSize(ir.size()),
      delayBuffer(std::max<std::size_t>(1, ir.size()), 0.0f), writeIndex(0)
{
  if (irSize == 0)
    throw std::invalid_argument("IR cannot be empty");
}
...
void TimeDomainConvolver::processBlock(const float *in, float *out,
